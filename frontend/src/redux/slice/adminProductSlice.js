...
export const createProduct = createAsyncThunk('adminProducts/createProduct', async(productData, {rejectWithValue}) => {
  try {
    ...
    return rejectWithValue(error.response?.data || "Failed to fetch createProduct")
  }
})
...
try {
  ...
  return rejectWithValue(error.response?.data || "Failed to fetch update data")
}
})
...
export const deleteProduct = createAsyncThunk('/adminProducts/deleteProduct', async(id, {rejectWithValue}) => {
  try {
    ...
    return rejectWithValue(error.response?.data || "Failed to fetch delete ")
  }
})
